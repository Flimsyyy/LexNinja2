...
{
    private const string HealAmount = "HealAmount";
    protected override IEnumerable<DynamicVar> CanonicalVars =>
        [
            new LexKelaVar(3),
            new PowerVar<GoodNightPower>(1),
            new CalculationBaseVar(0),
            new CalculationExtraVar(1),
            new CalculatedVar(HealAmount).WithMultiplier(
                (card, _) => HealRestSiteOption.GetHealAmount(card.Owner)
            ),
        ];
    ...
    protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay play)
    {
        ...
        await CommonActions.ApplySelf<GoodNightPower>(choiceContext, this);
